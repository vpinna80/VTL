...
public enum AnalyticOperator  
{
	...
	MIN(domain -> collectingAndThen(filtering(not(NullValue.class::isInstance), minBy(domain.getValueClass(), ScalarValue::compareTo)), opt -> opt.orElse(NullValue.instance(domain)))),
	MAX(domain -> collectingAndThen(filtering(not(NullValue.class::isInstance), maxBy(domain.getValueClass(), ScalarValue::compareTo)), opt -> opt.orElse(NullValue.instance(domain)))),
	VAR_POP(domain -> collectingAndThen(filtering(not(NullValue.class::isInstance), mapping(v -> (Number) v.get(), varianceCollector(acu -> acu[2] / (acu[0] + 1)))), NumberValueImpl::createNumberValue)),
	VAR_SAMP(domain -> collectingAndThen(filtering(not(NullValue.class::isInstance), mapping(v -> (Number) v.get(), varianceCollector(acu -> acu[2] / acu[0]))), NumberValueImpl::createNumberValue)),
