...
class SampleComponent<R extends Component, S extends ValueDomainSubset<S, D>, D extends ValueDomain> implements DataStructureComponent<R, S, D>
{
	...
	private class VariableView implements Variable<S, D>
	{
		...
		{
			return new SampleComponent<>(alias, role, domain);
		}
	...
	public SampleComponent(VTLAlias alias, Class<R> role, S domain)
	{
		this.alias = alias;
		this.role = role;
		this.domain = domain;
	...
	public DataStructureComponent<?, ?, ?> getRenamed(VTLAlias newAlias)
	{
		return new SampleComponent<>(newAlias, role, domain);
	}
}