...
public class InStrTransformation extends TransformationImpl
{
	...
	public VTLValue eval(TransformationScheme session)
	{
		...
		if (left.isDataSet())
		{
			...
			return dataset.mapKeepingKeys(structure, lineageEnricher(this), dp -> singletonMap(INT_VAR, 
	...
	protected VTLValueMetadata computeMetadata(TransformationScheme session)
	{
		...
		{
			DataSetStructure metadata = (DataSetStructure) left;
			
			if (right.isDataSet())
			{
				DataSetStructure patternMetadata = (DataSetStructure) right;
				final Set<? extends DataSetComponent<? extends Measure, ?, ?>> patternMeasures = patternMetadata.getMeasures();
			...
			return new DataSetStructureBuilder(metadata.getIDs())
					.addComponent(INT_VAR)
					.build();
		}
	}
